...
единственным выходом из этого места. Где-то здесь должен быть ключ...
        ...
        # Возвращаем инвентарь для передачи на уровень 2
...
def main():
    """Главная функция для запуска всей игры"""
    ...
    # Если игрок не вышел и прошел уровень 1, запускаем уровень 2
    if inventory is not None:
        print("\nПереход к следующему уровню...")
        time.sleep(2)
        
        # Запускаем уровень 2 с переданным инвентарем
...
class Level2:
    def __init__(self):
        self.player_inventory = []  # Список для инвентаря self ссылка на объект
        self.available_actions = ["осмотреть", "взять", "использовать", "читать", "инвентарь"]  # Список доступных действий
        self.interacted_objects = set()  # Множество взаимодействованных объектов
        
        # Словарь для описания объектов
        self.objects_description = {
            "стол": "Алхимический стол с колбами, ретортами и ступками",
            "флаконы": "Два флакона: голубоватый и багряный",
            "голубой_флакон": "Флакон с призрачным голубоватым сиянием",
        ...
        # Словарь характеристик игрока
        self.player_stats = {
            "здоровье": 100,
            "отравление": 30,  # Уровень отравления
            "статус": "отравлен"
        }
    ...
    def process_command(self, command):
        ...
        # Разделение команды на действие и объект
        parts = command.split()
        if len(parts) < 2:
            print("Не понял команду. Используйте: действие объект")
            return True
...
лишь усилит пламя, приведя к неминуемой гибели.{RESET}"""
    ...
    def drink_potion(self, potion_type):
        ...
        if potion_type == "голубой":
            ...
            print("\n🎉 ВЫ УСПЕШНО ПРОШЛИ 2 УРОВЕНЬ! 🎉")
            print("Элли исцелилась и готова к следующим приключениям!")
            return False  # Завершить уровень
        else:
            print("При первом же глотке жар в теле усиливается втрое!")
            print("Пламя жизни угасает, оставляя лишь ледяную пустоту...")
            self.player_stats["здоровье"] = 0
...
def main():
    """Главная функция для запуска игры (только уровни 3 и 4)"""
    print(f"{BOLD}=== ВОЛШЕБНИК ИЗУМРУДНОГО ГОРОДА ==={RESET}")
    time.sleep(2)
    ...
    # Если игрок не вышел и прошел уровень 3, запускаем уровень 4
    if inventory is not None:
        print("\nПереход к финальному уровню...")
        time.sleep(2)
        
        # Запускаем уровень 4 с переданным инвентарем
